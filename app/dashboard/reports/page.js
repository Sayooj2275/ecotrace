...
export default function ReportsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8 font-sans text-gray-800 print:bg-white print:p-0">
      
      {/* 🟢 print:hidden hides this top section in the PDF */}
      ...
      {/* 🟢 print:hidden hides the filters */}
      ...
      {/* 📄 THE REPORT PAPER UI */}
      {/* 🟢 print:shadow-none removes the 3D effect on paper for printing */}
      <div className="max-w-4xl mx-auto bg-white shadow-xl border border-gray-100 min-h-[1000px] p-12 relative print:shadow-none print:border-none print:w-full print:max-w-none print:p-8">
        ...
        {/* Disclaimer */}
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-8 text-xs text-gray-500 leading-relaxed flex gap-3 print:bg-gray-100">
        ...
        <div className="grid grid-cols-3 gap-4 mb-10">
            <div className="p-4 border border-gray-100 rounded-lg bg-white shadow-sm print:border-gray-300">
            ...
            <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50">
                    {loading ? (
                        ...
                        <tr key={log.id} className="group hover:bg-gray-50 transition print:break-inside-avoid">
                            <td className="py-4 text-sm font-medium text-gray-500">{new Date(log.created_at).toLocaleDateString()}</td>
                            <td className="py-4 text-sm font-bold text-gray-800">{log.type}</td>
                            <td className="py-4 text-sm text-gray-500">{log.collected_weight ? `Verified pickup: ${log.collected_weight}kg` : `Request: ${log.weight}kg`}</td>
                            <td className="py-4 text-right">
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium border ${
                                    log.status === 'collected' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-yellow-50 text-yellow-700 border-yellow-100'
                                }`}>
        ...
        <div className="mb-10 border border-blue-100 bg-blue-50/50 rounded-xl p-6 print:bg-blue-50 print:border-none">
